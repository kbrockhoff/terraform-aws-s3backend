...
variable "name_prefix" {
  ...
  validation {
    ...
    error_message = "The name_prefix value must start with a lowercase letter, followed by 0 to 22 alphanumeric or hyphen characters, ending with an alphanumeric character, for a total length of 2 to 24 characters."
  }
}
...
variable "cost_estimation_config" {
  description = "Configuration object for cost estimation settings"
  type = object({
    enabled = bool
  })
  default = {
...
variable "monitoring_config" {
  description = "Configuration object for monitoring settings"
  type = object({
    enabled = bool
  })
  default = {
